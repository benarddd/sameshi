...
// Independence Day Component (28-29 November)
function IndependenceDayContent() {
  return (
...
// Regular Home Page Component
function RegularHomePage({ showModal, setShowModal }: { showModal: boolean; setShowModal: (show: boolean) => void }) {
  return (
...
// Main Ballina Component with Date Check
export default function Ballina() {
  const [showModal, setShowModal] = useState(false);
  const [isIndependenceDay, setIsIndependenceDay] = useState(false);

  useEffect(() => {
    // Check if today is November 28 or 29
    const today = new Date();
    const month = today.getMonth(); // 0-indexed, so November is 10
  ...
  // Show Independence Day content on November 28-29
  if (isIndependenceDay) {
    return <IndependenceDayContent />;
  }

  // Show regular home page on all other days
  return <RegularHomePage showModal={showModal} setShowModal={setShowModal} />;
}
